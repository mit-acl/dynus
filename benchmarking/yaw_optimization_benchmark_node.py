...
class BenchmarkNode(Node):
    ...
    def __init__(self):
        ...
        # self.global_algorithm = 'dastar'

        # Benchmarking types
        self.benchmark_type = 'yaw_optimization'
        self.algorithms = ['dynus']
        # self.benchmark_names = ['benchmark1', 'benchmark2'] # benchmark1: easy dyn obstacles and benchmark2: hard dyn obstacles
        self.benchmark_names = ['benchmark2'] # benchmark1: easy dyn obstacles and benchmark2: hard dyn obstacles
    ...
    def run_simulation(self):
        ...
        for benchmark_name in self.benchmark_names:
            ...
            for algorithm in self.algorithms:
                ...
                csv_folder_path = f"/media/XXXXXXX/T7/dynus/tro_paper/{self.benchmark_type}/{benchmark_name}/csv/{algorithm}"
                bag_folder_path = f"/media/XXXXXXX/T7/dynus/tro_paper/{self.benchmark_type}/{benchmark_name}/bags/{algorithm}"
                log_folder_path = f"/media/XXXXXXX/T7/dynus/tro_paper/{self.benchmark_type}/{benchmark_name}/logs/{algorithm}"
                ...
                for i in range(self.start_iter_num, self.iterations):
                    ...
                    self.start_simulation(self.global_algorithm, csv_folder_path, bag_folder_path, algorithm, benchmark_name)
    ...
    def start_simulation(self, global_planner_algorithm='dgp', csv_folder_path='', bag_folder_path='', algorithm='dynus', benchmark_name='benchmark1'):
        ...
        # Base
        self.sim_process_base = subprocess.Popen(["ros2", "launch", "dynus", "base_dynus.launch.py", f"use_dyn_obs:={self.use_dyn_obs}", "use_gazebo_gui:=false", f"use_rviz:={self.use_rviz}", "use_ground_robot:=false", f"benchmark_name:={benchmark_name}", "env:=empty"], preexec_fn=os.setsid)
