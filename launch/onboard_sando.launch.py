...
def generate_launch_description():
    ...
    data_file_arg = DeclareLaunchArgument('data_file', default_value='/media/kkondo/T7/sando/tro_paper/global_planner_benchmarking/dgp.csv', description='File name to store data') # file name to store data
    ...
    skip_initial_yawing_arg = DeclareLaunchArgument('skip_initial_yawing', default_value='', description='Skip YAWING state and go directly to TRAVELING (true/false). Empty string uses value from config file.')
    ...
    def launch_setup(context, *args, **kwargs):
        ...
        skip_initial_yawing_override = LaunchConfiguration('skip_initial_yawing').perform(context)
        ...
        urdf_path=PathJoinSubstitution([FindPackageShare('sando'), 'urdf', 'quadrotor.urdf.xacro'])
        if use_hardware:
            parameters_path=os.path.join(get_package_share_directory('sando'), 'config', 'sando_hw_quadrotor.yaml')
        else:
            parameters_path=os.path.join(get_package_share_directory('sando'), 'config', 'sando.yaml')
        ...
        if skip_initial_yawing_override:
            parameters['skip_initial_yawing'] = convert_str_to_bool(skip_initial_yawing_override)
