        ...
        if monitor.obstacle_trajectories and len(monitor.positions) > 0:
            try:
                ...
                for i, t_abs in enumerate(monitor.timestamps):
                    ...
                    for obs_id, obs_traj in monitor.obstacle_trajectories.items():
                        # Interpolate obstacle position at this time
                        obs_pos = interpolate_obstacle_position(
                            obs_traj['times'],
                            obs_traj['positions'],
                            t_abs
                        )
                        ...
                        if (obs_pos[0] - hx <= px <= obs_pos[0] + hx and
                            ...
                            collision = CollisionEvent(
                                ...
                                obstacle_half_extents=(hx, hy, hz)
                            )
                ...
                coll_stats = {
                    ...
                    'max_penetration': max((c.penetration_depth for c in collisions), default=0.0),
                    'avg_penetration': sum(c.penetration_depth for c in collisions) / len(collisions) if collisions else 0.0,
                }

                # Update metrics with collision data
                metrics.collision_count = coll_stats['total_collisions']
                metrics.collision_penetration_max = coll_stats['max_penetration']
                metrics.collision_penetration_avg = coll_stats['avg_penetration']
                metrics.collision_unique_obstacles = coll_stats['unique_obstacles_hit']
                metrics.collision_free_ratio = coll_stats['collision_free_ratio']
                metrics.collision = (coll_stats['total_collisions'] > 0)
                metrics.min_distance_to_obstacles = min_distance if min_distance != float('inf') else 0.0

                print(f"  Collision check: {coll_stats['total_collisions']} events, "
                      f"{coll_stats['unique_obstacles_hit']} obstacles hit, "
                      f"min distance: {metrics.min_distance_to_obstacles:.3f}m")

                # Debug: Print detailed info for first 3 collisions if any detected
                if collisions and coll_stats['total_collisions'] > 0:
                    print(f"\n  Collision Details (showing first 3 of {len(collisions)}):")
                    for i, coll in enumerate(collisions[:3]):
                        print(f"    [{i+1}] Time: {coll.time:.2f}s")
                        print(f"        Drone pos: ({coll.drone_position[0]:.2f}, {coll.drone_position[1]:.2f}, {coll.drone_position[2]:.2f})")
                ...
                print(f"  Warning: Collision checking failed: {e}")
                import traceback
                traceback.print_exc()
