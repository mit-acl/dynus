...
def print_summary(df: pd.DataFrame, name: str = "Dataset"):
    ...
    if len(success_df) > 0:
        print(f"\nSuccessful Trials:")
        print(f"  Travel time:      {success_df['flight_travel_time'].mean():.2f} ± {success_df['flight_travel_time'].std():.2f}s")
        print(f"  Path length:      {success_df['path_length'].mean():.2f} ± {success_df['path_length'].std():.2f}m")
        print(f"  Path efficiency:  {success_df['path_efficiency'].mean():.3f} ± {success_df['path_efficiency'].std():.3f}")
        print(f"  Avg velocity:     {success_df['avg_velocity'].mean():.2f} ± {success_df['avg_velocity'].std():.2f} m/s")
        print(f"  Max velocity:     {success_df['max_velocity'].mean():.2f} ± {success_df['max_velocity'].std():.2f} m/s")

        print(f"\n  Smoothness:")
        print(f"  Jerk integral:    {success_df['jerk_integral'].mean():.2f} ± {success_df['jerk_integral'].std():.2f}")
        print(f"  RMS jerk:         {success_df['jerk_rms'].mean():.2f} ± {success_df['jerk_rms'].std():.2f}")
        ...
        print(f"  Velocity:         {vel_viol} ({vel_viol/len(success_df):.1f} per trial)")
        print(f"  Acceleration:     {acc_viol} ({acc_viol/len(success_df):.1f} per trial)")
        print(f"  Jerk:             {jerk_viol} ({jerk_viol/len(success_df):.1f} per trial)")
...
def plot_results(df: pd.DataFrame, output_path: str = None):
    ...
    ax1.axvline(success_df['flight_travel_time'].mean(), color='r', linestyle='--', label='Mean')
    ax1.legend()

    # 2. Path efficiency distribution
    ax2 = fig.add_subplot(gs[0, 1])
    ax2.hist(success_df['path_efficiency'], bins=20, edgecolor='black', alpha=0.7, color='green')
    ax2.set_title('Path Efficiency Distribution', fontweight='bold')
    ax2.set_xlabel('Efficiency (straight_line / path_length)')
    ax2.set_ylabel('Count')
    ax2.axvline(success_df['path_efficiency'].mean(), color='r', linestyle='--', label='Mean')
    ax2.legend()
    ...
    scatter = ax4.scatter(success_df['num_obstacles'], success_df['flight_travel_time'],
                         c=success_df['path_efficiency'], cmap='RdYlGn', alpha=0.6, s=50)
    ...
    ax5.scatter(success_df['path_length'], success_df['path_efficiency'], alpha=0.6, s=50)
    ...
    ax6.scatter(success_df['path_length'], success_df['jerk_rms'], alpha=0.6, s=50, color='purple')
    ...
    ax7.hist(success_df['avg_velocity'], bins=20, alpha=0.5, label='Average', edgecolor='black')
    ax7.hist(success_df['max_velocity'], bins=20, alpha=0.5, label='Maximum', edgecolor='black')
    ...
    ax8.bar(violations.keys(), violations.values(), color=colors, edgecolor='black', alpha=0.7)
    ...
    if success_df['dynamic_ratio'].nunique() > 1:
        ...
        ax9.hist(success_df['jerk_integral'], bins=20, edgecolor='black', alpha=0.7, color='orange')
...
def main():
    ...
    parser.add_argument(
        'data',
        type=str,
        help='Path to benchmark CSV file(s) (supports wildcards)'
    )
