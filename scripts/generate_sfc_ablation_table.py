...
def generate_table(rows):
    ...
    """
    metrics = ['success_rate', 'comp_time', 'travel_time', 'path_length',
               'jerk_integral', 'min_dist', 'vel_viol', 'acc_viol', 'jerk_viol']
    ...
    lines.append(r"  \caption{SFC ablation study. We compare Worst-Case SFC and Spatio-Temporal SFC (STSFC) at different maximum velocities in the Hard dynamic environment. We highlight the \best{best} and \worst{worst} value for each velocity.}")
    ...
    for vi, vel in enumerate(sorted_vels):
        ...
        for ri, (sfc_mode, stats) in enumerate(group):
            ...
            parts.append(f" & {wrap(fmt(stats['success_rate'], 1), vel, sfc_mode, 'success_rate')}")
            parts.append(f" & {wrap(fmt(stats['comp_time'], 1), vel, sfc_mode, 'comp_time')}")
            parts.append(f" & {wrap(fmt(stats['travel_time'], 1), vel, sfc_mode, 'travel_time')}")
            parts.append(f" & {wrap(fmt(stats['path_length'], 1), vel, sfc_mode, 'path_length')}")
            parts.append(f" & {wrap(fmt(stats['jerk_integral'], 1), vel, sfc_mode, 'jerk_integral')}")
            parts.append(f" & {wrap(fmt(stats['min_dist'], 2), vel, sfc_mode, 'min_dist')}")
            parts.append(f" & {wrap(fmt(stats['vel_viol'], 1), vel, sfc_mode, 'vel_viol')}")
            parts.append(f" & {wrap(fmt(stats['acc_viol'], 1), vel, sfc_mode, 'acc_viol')}")
            parts.append(f" & {wrap(fmt(stats['jerk_viol'], 1), vel, sfc_mode, 'jerk_viol')}")
            parts.append(" \\\\")
...
def main():
    ...
    for sfc_mode, vel, data_dir in CONFIGS:
        ...
        print(f"  Success: {stats['success_rate']}%, Comp: {stats['comp_time']}ms, "
