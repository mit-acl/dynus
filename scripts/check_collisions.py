...
class BoundingBox:
    ...
    @staticmethod
    def from_center_and_half_extents(cx: float, cy: float, cz: float,
                                     hx: float, hy: float, hz: float) -> 'BoundingBox':
...
class CollisionChecker:
    ...
    def __init__(self, drone_half_extents: Tuple[float, float, float],
    ...
    def check_trajectory(self, trajectory: pd.DataFrame, obstacles: List[Obstacle],
                        dt: float = 0.01) -> Tuple[List[CollisionEvent], Dict]:
        """
        ...
        for idx, row in trajectory.iterrows():
            ...
            drone_bbox = BoundingBox.from_center_and_half_extents(
                px, py, pz,
                self.drone_hx, self.drone_hy, self.drone_hz
            )
...
def load_drone_bbox_from_yaml(yaml_path: str) -> Tuple[float, float, float]:
    ...
    if drone_bbox_full is None:
        print("Warning: drone_bbox not found in YAML, using default full size [0.6, 0.6, 0.3]")
        drone_bbox_full = [0.6, 0.6, 0.3]

    # Convert from full size to half-extents
    half_extents = (drone_bbox_full[0] / 2.0, drone_bbox_full[1] / 2.0, drone_bbox_full[2] / 2.0)
    return half_extents


def save_collision_report(collisions: List[CollisionEvent], stats: Dict,
                          output_path: str, trajectory_path: str):
    ...
    with open(txt_path, 'w') as f:
        ...
        for i, c in enumerate(collisions[:100]):  # Limit to first 100 for readability
            f.write(f"[{i+1}] t={c.time:.3f}s, obstacle={c.obstacle_id}, "
                   f"penetration={c.penetration_depth:.4f}m\n")
            f.write(f"    Drone pos: ({c.drone_position[0]:.2f}, "
                   f"{c.drone_position[1]:.2f}, {c.drone_position[2]:.2f})\n")
            f.write(f"    Obs pos: ({c.obstacle_position[0]:.2f}, "
                   f"{c.obstacle_position[1]:.2f}, {c.obstacle_position[2]:.2f})\n")
            f.write(f"    Obs size: ({c.obstacle_half_extents[0]:.2f}, "
...
def main():
    ...
    checker = CollisionChecker(
        drone_half_extents=drone_half_extents,
    ...
    print(f"Collision-free segments: {stats['collision_free_segments']} / {stats['total_segments']}")
