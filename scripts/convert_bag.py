...
def filter_pc_z(msg: PointCloud2, z_max: float) -> PointCloud2:
    ...
    return point_cloud2.create_cloud(msg.header, msg.fields, filtered if filtered else [])
...
def main():
    parser = argparse.ArgumentParser(description='Convert bag: adjust lifetimes + z-filter pointclouds')
    parser.add_argument('input_bag', help='Path to input bag directory')
    parser.add_argument('--lifetime', type=float, default=0.1,
                        help='Marker lifetime in seconds (default: 0.1)')
    parser.add_argument('--z-max', type=float, default=None,
                        help='Max z for pointcloud filtering (disabled if not set)')
    parser.add_argument('--pc-topics', nargs='*', default=[],
                        help='PointCloud2 topics to z-filter')
    ...
    while reader.has_next():
        ...
        elif type_str == 'sensor_msgs/msg/PointCloud2' and topic in pc_topics_set and args.z_max is not None:
