...
void DGPPlanner::setMapUtil(const std::shared_ptr<sando::MapUtil<3>> &map_util)
{
...
bool DGPPlanner::plan(const Vecf<3> &start, const Vecf<3> &start_vel, const Vecf<3> &goal, double &final_g, double current_time, decimal_t eps)
{
  ...
  graph_search_ = std::make_shared<sando::GraphSearch>((map_util_->map_).data(), map_util_, dim(0), dim(1), dim(2), eps, planner_verbose_, global_planner_, w_unknown_);
