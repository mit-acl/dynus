...
struct ObstacleSpec
{
    ...
    void compile()
    {
        ...
        if (!has_t)
        {
            throw std::runtime_error("Compiled expression does not reference variable 't' (constant expression?)");
        }
...
public:
    DynamicForestNode() : Node("dynamic_forest_trajs")
    {
        ...
        std::string obstacles_json_str =
            declare_parameter<std::string>("obstacles_json", "[]");
        ...
        if (publish_markers_)
        {
            dyn_mesh_pub_ = create_publisher<visualization_msgs::msg::MarkerArray>("/shapes_dynamic_mesh", 10);
...
private:
    ...
    {
        if (!use_spawn_origins_ || adopted_spawn_)
            return;
        ...
        else
        {
            RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 3000,
                                 "Spawn adoption incomplete (%d/%zu). Continuing...",
                                 matched, obstacles_.size());
        }
    }
    ...
    visualization_msgs::msg::Marker makeMeshMarker(int id,
                                                   const std::array<double, 3> &bbox,
    ...
    void timerCB()
    {
        ...
        if (publish_markers_)
            points_marker = makePointsMarker();
