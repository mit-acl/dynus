...
struct DynObs
{
    ...
    {
        if (!compiled) { x = x0; y = y0; z = z0; return; }
        t_var = t;
        x = ex.value();
        y = ey.value();
        z = ez.value();
...
public:
    void Load(physics::WorldPtr world, sdf::ElementPtr sdf) override
    {
        ...
        if (sdf->HasElement("json_path"))
            json_path_ = sdf->Get<std::string>("json_path");
...
private:
    ...
    bool LoadJSON()
    {
        ...
        try
        {
            ...
            gzmsg << "[DynObsPlugin] Loaded " << obstacles_.size()
                  << " obstacles from " << json_path_ << "\n";
            json_loaded_ = true;
            return true;
        }
    ...
    void SpawnModels()
    {
        ...
        gzmsg << "[DynObsPlugin] Queued " << obstacles_.size()
              << " box models for insertion\n";
    }
    ...
    bool ResolveModels()
    {
        ...
        {
            if (!o.model)
                o.model = world_->ModelByName(o.name);
            if (o.model)
                ++found;
        }
        if (found < static_cast<int>(obstacles_.size()))
        {
            ...
            if (++resolve_counter_ % 1000 == 0)
                gzmsg << "[DynObsPlugin] Resolving models: " << found
                      << "/" << obstacles_.size() << "\n";
            return false;
        }
        return true;
