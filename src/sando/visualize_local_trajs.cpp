...
{
    // SANDO2
    if (planner_key == "sando_N4" || planner_key == "sando_N4")
        return "SANDO2(N=4)";
    if (planner_key == "sando_N5" || planner_key == "sando_N5")
        return "SANDO2(N=5)";
    if (planner_key == "sando_N6" || planner_key == "sando_N6")
        return "SANDO2(N=6)";
...
public:
    ...
    {
        // Inputs
        sfc_dir_ = declare_parameter<std::string>("sfc_dir", "/home/kkondo/code/dynus_ws/src/sando/data");
        file_ext_ = declare_parameter<std::string>("file_ext", ".mysco2");
        traj_dump_root_dir_ = declare_parameter<std::string>("traj_dump_root_dir", "/home/kkondo/code/dynus_ws/src/sando/benchmark_data/single_thread/traj_dump");

        // New: multiple roots
        traj_dump_root_dirs_ = declare_parameter<std::vector<std::string>>(
            "traj_dump_root_dirs",
            std::vector<std::string>{
                traj_dump_root_dir_,                               // sando/faster dumps
