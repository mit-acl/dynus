...
namespace sando_utils
{
  ...
  std::vector<double> getTravelTimes(const vec_Vecf<3> &path, const state &A, bool debug_verbose, const Eigen::Vector3d &v_max_3d, const Eigen::Vector3d &a_max_3d)
  {
    ...
    {
      double travel_time = sando_utils::getMinTimeDoubleIntegrator3D(path[0], A.vel, path[1], Eigen::Vector3d::Zero(), v_max_3d, a_max_3d);
      return {travel_time};
    }
    else
    {
    ...
    for (int i = 0; i < path.size() - 1; i++)
    {
      ...
      // Compute travel time
      double travel_time = sando_utils::getMinTimeDoubleIntegrator3D(path[i], velocities[i], path[i + 1], velocities[i + 1], v_max_3d, a_max_3d);
