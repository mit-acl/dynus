...
{
    if (v.size() != 3)
        throw std::runtime_error("Parameter '" + name + "' must have size 3.");
...
static state makeStateFromPos(const Vec3f &p)
{
    state s;
...
static double unionViolationAtLayer(const std::vector<LinearConstraint3D> &polys_at_n, const Vec3f &p, int *best_poly = nullptr)
{
...
public:
    TemporalLayeredCorridorTestNode() : Node("temporal_layered_corridor_test_node")
    {
        ...
        // Corridor generation params (DGPManager / EllipsoidDecomp)
        declare_parameter<double>("res", 0.15);
        declare_parameter<double>("factor_dgp", 1.0);
        declare_parameter<double>("inflation_dgp", 0.0);
        ...
        declare_parameter<double>("dgp_timeout_ms", 50.0);
        ...
        // Build parameters struct used by both DGPManager and SolverGurobi
        par_.num_N = N_local_;
        par_.dc = dc_;
        ...
        {
            auto lbs = get_parameter("local_box_size").as_double_array();
            if (lbs.size() != 3)
                throw std::runtime_error("local_box_size must be size 3");
            par_.local_box_size = {lbs[0], lbs[1], lbs[2]};
        }
        ...
        dgp_timeout_ms_ = get_parameter("dgp_timeout_ms").as_double();
        global_planner_hweight_ = get_parameter("global_planner_huristic_weight").as_double();
        ...
        pub_poly_layer_ = create_publisher<decomp_ros_msgs::msg::PolyhedronArray>(poly_topic_, qos_poly);
        ...
        pub_dgp_path_ = create_publisher<visualization_msgs::msg::MarkerArray>(dgp_path_topic_, qos_latched);
        pub_traj_colored_ = create_publisher<visualization_msgs::msg::MarkerArray>(traj_colored_topic_, 10);
...
private:
    ...
    void buildGlobalPath_()
    {
        ...
        const bool ok = dgp_.solveDGP(start_, dir, goal_, final_g, global_planner_hweight_, tnow, path, raw_path);

        if (!ok || path.size() < 2)
            throw std::runtime_error("DGP global planner failed to find a path.");
    ...
    void buildTimeLayers_()
    {
        ...
        for (int n = 0; n < N_local_; ++n)
            time_end_times_.push_back((double)(n + 1) * dt_layer);
    }

    void buildTemporalCorridor_()
    {
        // DGP manager parameter wiring
        dgp_.setParameters(par_);
        ...
        const bool ok = dgp_.cvxEllipsoidDecompTimeLayered(
            ellip,
            global_path_,
            base_uo_,
            obst_pos_,
        ...
        if (!ok)
            throw std::runtime_error("cvxEllipsoidDecompTimeLayered failed.");
    }
    ...
    void solveTrajectory_()
    {
        ...
        solver_->setPlannerName("DYNUS");
        solver_->initializeSolver(par_);
        solver_->setT0(0.0);
        solver_->setInitialDt(initial_dt_);

        // Boundary conditions
        const state x0 = makeStateFromPos(start_);
        state xf;
        xf.setPos(global_path_[(size_t)P_global_ - 1].x(),
                  global_path_[(size_t)P_global_ - 1].y(),
        ...
        const bool ok = solver_->generateNewTrajectory(gurobi_error, grb_ms, factor_, use_single_thread);
    ...
    void runPostCheck_()
    {
        ...
        RCLCPP_INFO(get_logger(),
                    "Post-check (sampled @dc=%.3f): worst union violation = %.6f at t=%.3f (layer n=%d, best_poly=%d) p=(%.3f,%.3f,%.3f). "
                    "Interpretation: <=0 means inside corridor union for that layer.",
                    dc_, worst, t_at_worst, n_at_worst, best_poly,
                    p_at_worst.x(), p_at_worst.y(), p_at_worst.z());
    }
    ...
    void publishOnce_()
    {
        ...
        // Start / goal
        ma.markers.push_back(makeSphere(frame_id_, 1, start_, 0.20, colorRGBA(0.0f, 1.0f, 0.0f, 1.0f), "start_goal", stamp));
        ma.markers.push_back(makeSphere(frame_id_, 2, goal_, 0.20, colorRGBA(1.0f, 0.0f, 0.0f, 1.0f), "start_goal", stamp));

        // Global path
        // ma.markers.push_back(makeLineStrip(frame_id_, 10, global_path_line_, 0.04, colorRGBA(1.0f, 1.0f, 1.0f, 0.6f), "global_path", stamp));

        // Trajectory
        // ma.markers.push_back(makeLineStrip(frame_id_, 20, traj_line_, 0.06, colorRGBA(0.2f, 0.6f, 1.0f, 0.9f), "local_traj", stamp));
        ...
        for (size_t i = 0; i < obst_pos_.size(); ++i)
        {
            ...
            if (r > 1e-6)
            {
                ma.markers.push_back(makeSphere(frame_id_, 2000 + (int)i, obst_pos_[i], diameter,
                                                colorRGBA(1.0f, 0.6f, 0.0f, 0.12f), "dyn_obst_reachable", stamp));
            }
        }
        ...
        auto traj_ma = stateVector2ColoredMarkerArray(
            goal_setpoints_,
            /*type*/ 0,
            /*max_value*/ par_.v_max,
            stamp,
            frame_id_);
        pub_traj_colored_->publish(traj_ma);

        // Publish global path to /NX01/dgp_path_marker as thin line + dots
        visualization_msgs::msg::MarkerArray dgp_ma;
...
private:
    ...
    rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr pub_dgp_path_;
